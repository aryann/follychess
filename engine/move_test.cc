...
TEST(Move, Null) {
  ...
  EXPECT_THAT(move.IsNullMove(), IsTrue());
...
TEST(FromUCI, Valid) {
  {
    std::expected<Move, std::string> move = Move::FromUCI("0000");
  ...
  }
  {
    ...
    ASSERT_THAT(move.error_or(""), IsEmpty());
    EXPECT_THAT(move->IsNullMove(), IsFalse());
...
TEST(Move, String) {
  ...
  EXPECT_THAT(ToString(Move::NullMove()), Eq("0000"));
